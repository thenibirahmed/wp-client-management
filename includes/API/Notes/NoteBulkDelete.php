...
class NoteBulkDelete {

    private $namespace = 'wp-client-management/v1';
    private $endpoint = '/notes/bulk-delete';
    ...
    public function __construct() {
        register_rest_route($this->namespace, $this->endpoint, [
            ...
            'callback' => [$this, 'bulk_delete_note'],
            'permission_callback' => 'is_user_logged_in',
        ]);
    }

    public function bulk_delete_note(\WP_REST_Request $request) {
        global $validator;
        ...
        $bulk_delete_notes = Note::whereIn('id', $bulk_ids)->get();

        if ($bulk_delete_notes->isEmpty()) {
            return new \WP_REST_Response([
        ...
        try {
            foreach ($bulk_delete_notes as $note) {
                $note->delete();
            }
        } catch (\Exception $e) {
            return new \WP_REST_Response([
